...
class MedMNISTDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.mnist_train = self.dataset_class(
                split='train', transform=self.default_transform(), size=self.size, download=True
            )  # noqa: B026
            self.mnist_val = self.dataset_class(
                split='val', transform=self.default_transform(), size=self.size, download=True
            )  # noqa: B026

        # Assign test dataset for use in dataloader(s)
        if stage == 'test' or stage is None:
            self.mnist_test = self.dataset_class(
                split='test', transform=self.default_transform(), size=self.size, download=True
            )  # noqa: B026
